...
print "Before running this script, make sure that you have edited the file \"Genotypes/Output/SNPMatrix_X.Y.txt\" so that the first sample in the file is an outgroup individual, and the remaining ingroup";
print " samples are grouped by population.\n\n"; 
...
for my $argument (@ARGV)  {
	...
	if (($TempArgs[1] =~ /^h$/)|| ($TempArgs[1] =~ /^help$/)) {
		...
		print "Information for OutputFastSimCoal_JointSFS.pl...\n\n";
		print "This script creates unfolded joint site frequency spectra that can be used as input for analyses in FastSimCoal\n";  
...
print "Enter the number of individuals in each population, in the order the populations occur in the SNPMatrix_X.Y.txt file.  Separate each with a tab.  Do not include the outgroup sample.\n";
my $PopSizes = <STDIN>;
chomp($PopSizes);
...
##############################################################################################################################################
		...
		open MONOMORPHICINALL, "../Output/Genotypes/Monomorphics_100.txt" or die$!;	
		...
		my $Read = $Monomorphics[2];
		my $ReadLength = length($Read);
		my $MonomorphicSites = 	$NumMonomorphicLoci*$ReadLength;
		
		#Adjust number of monomorphic sites based on the percent of loci sampled for the resampled dataset.
		
		$MonomorphicSites = int(($PctLoci/100)*$MonomorphicSites);

		my $NumSitesInPolymorphicReads = $NumPolymorphicLoci*$ReadLength;

		my $NumMonomorphicSitesInPolymorphicLoci = $NumSitesInPolymorphicReads-$TotalNumSNPs;

		$MonomorphicSites = $MonomorphicSites+$NumMonomorphicSitesInPolymorphicLoci;
...
##############################################################################################################################################
		...
		foreach my $locusname (@UnlinkedBiallelicsNames) {
			...
			print OUTFILE "$MonomorphicSites\t";
...
##############################################################################################################################################
	...
	foreach my $pop (0..$NumPops-2) {			#Do pairwaise comparisons for each pair of populations.
		...
		foreach my $comparison ($pop+1..$NumPops-1)  {
			...
			open OUTFILE, ">jointDAFpop$CurrentComparison.obs" or die$!;		#Create file for current pair of populations to store DFS.
			...
			my $TotalMonomorphicSites = $CurrentMonomorphicSitesSum+$MonomorphicSites;
			
			$DerivedCountsHash{'0a0'} = $TotalMonomorphicSites;
