...
LocationsToKeep<-scan(file="TempFiles/SitesToRetain.txt")

DataTableFromFile<-read.table("TempFiles/TempSNPMatrix.txt", header=TRUE, sep = "\t")  #####Have to edit this line.
...
write.table(MatrixToPrint,file="TempFiles/SNPMatrix_GoodLocations.txt",sep="\t",row.names=FALSE, col.names=FALSE)