...
if ($Subset == 1)  {
	...
	while (<CURRENTBARCODEFILE>)  {
		...
		if (-e "TempFiles/ForBinomialTest$TabSeparatedArray[0].txt")  {
...
if ($MaxProcesses > 1)  {
	...
	if ($MaxProcesses > $LengthAllSamples)  {
		...
		foreach my $number (0..$LengthAllSamples-1)  {
			mkdir "TempFiles/SubGenotypes_$number";
			push (@SubprocessDirectories, "SubGenotypes_$number");
			open GENS, ">TempFiles/SubGenotypes_$number/SubGenotypes.txt" or die$!;
			open NAMES, ">TempFiles/SubGenotypes_$number/SubNames.txt" or die$!;
			close GENS;
			close NAMES;
		}
	}	
	...
	while (<CURRENTBARCODEFILE>)  {
		...
		if (-e "TempFiles/ForBinomialTest$TabSeparatedArray[1].txt")  {
			open NAMES, ">>TempFiles/SubGenotypes_$LineCounter/SubNames.txt" or die$!;
	...
	foreach my $subdir (@SubprocessDirectories)  {	
		...
		open NAMES, "TempFiles/$subdir/SubNames.txt" or die $!;
		
		open GENOTYPESFILE, ">>TempFiles/$subdir/SubGenotypes.txt" or die$!;
		
		open TEMPGEN, ">TempFiles/$subdir/TempGenotypes.txt" or die$!;
		...
		while(<NAMES>) {
			...
			if ($~ =~ /[A-Za-z0-9]/) {
				...
				open LOCIFILE, "TempFiles/ForBinomialTest$CurrentName.txt" or die$!;
				open TEMPLOCIFILE, ">TempFiles/$subdir/TempBiallelicLociForGenotyping.txt" or die$!;
				...
				system "cp RScripts/Genotype_Edit.R TempFiles/$subdir";
				
				
				
				open GENOTYPERSCRIPT, "TempFiles/$subdir/Genotype_Edit.R" or die$!;
				open GENOTYPERSCRIPTEDIT, ">TempFiles/$subdir/Genotype_SubEdit.R" or die$!;
				 
				 
				while(<GENOTYPERSCRIPT>)  {
					 if ($_ =~ /Genotypes<-read/) {
						 print GENOTYPERSCRIPTEDIT "Genotypes<-read.table(file=\"TempFiles/$subdir/TempBiallelicLociForGenotyping.txt\")\n";
						 next;
					 }
					 
					 elsif ($_ =~ /TempGenotypes/)  {
						my $SubstituteText = 'TempFiles/'.$subdir.'/TempGenotypes';
				...
				system "R --vanilla --slave < TempFiles/$subdir/Genotype_SubEdit.R";
				
				print "Finished R\n";
	
				open TEMPGENOTYPES, "TempFiles/$subdir/TempGenotypes.txt" or die$!;
				my $LineCounter = 0;
				...
				system "rm TempFiles/$subdir/TempGenotypes.txt";
	...
	open GENOTYPESFILE, ">TempFiles/GenotypesLocusNamesForCat.txt" or die$!;
	...
	for my $filenumber (0..$NumSubdirectories-1) {		
		$FilesToCat = $FilesToCat."TempFiles/SubGenotypes_$filenumber/SubGenotypes.txt ";		
	}
		
	system "cat TempFiles/GenotypesLocusNamesForCat.txt $FilesToCat > TempFiles/Genotypes.txt";
...
open REMOVEDSAMPLES, ">TempFiles/MasterReport.txt" or die$!;
...
open GOODSAMPLES, ">TempFiles/GoodSampleNames.txt" or die$!;
...
system "mv TempFiles/MonomorphicUpdate$FinalNumber.txt TempFiles/MonomorphicLociWithCounts.txt";
...
my $Monomorphics;	#Line count from TempFiles/MonomorphicLoci.txt
...
open MASTER, ">>TempFiles/MasterReport.txt" or die$!;
print MASTER "\n\n";
...
system "wc -l TempFiles/MonomorphicsNoParalogs.txt | tee -a TempFiles/MasterReport.txt";
print MASTER "\n";
...
print "The locations of each SNP are plotted in the file SNPLocations.pdf in the Output/RunInfo folder.  Use this plot to choose a cut-off value for omitting SNPs due to artifacts toward the ends of reads.\n\n";
