...
open ALLSAMPLENAMES, ">TempFiles/TotalSampleNames.txt" or die$!;
open MINALIGNLENGTH, ">TempFiles/MinAlignLengthFile.txt" or die$!;
...
if ($MaxProcesses > 1) {
	...
	if ($MaxProcesses > $NumOfRuns) {
		...
		foreach my $number (0..$NumOfRuns-1)  {
			...
			mkdir "TempFiles/SubTemp_$number";
...
# 
	...
	foreach my $number (@DirectoriesNumbers)  {	
		...
		#mkdir "TempFiles/SubTemp$folder";
	...
	##################################################################################################################################################################################################
			...
			open GOODSEQSGOODBARCODES, ">TempFiles/SubTemp_$number/AllGoodSeqsWithAssignableBarcodes$FileNumber.txt" or die$!;
			open GOODSEQSNOBARCODES, ">>TempFiles/SubTemp_$number/GoodSeqsNoBarcodes.txt" or die$!;
	...
	#############################################################################################################################################################################
			...
			open REPORT, ">Output/RunInfo/Report_$SubFileNamesWExtension[$FileNumber-1].txt"; 
	...
	#############################################################################################################################################################################
			...
			open BARCODES, ">Output/RunInfo/BarcodeInfo_$SubFileNamesWExtension[$FileNumber-1].txt";
			#open BARCODECOUNTSTOPLOT, ">TempFiles/SubTemp_$number/BarcodeCountsToPlot.txt" or die$!;
			#open BARCODENAMESTOPLOT, ">TempFiles/SubTemp_$number/BarcodeNamesToPlot.txt" or die$!;
	...
	############################################################################################################################################################################
		...
		open UNSORTED, "TempFiles/SubTemp_$number/AllGoodSeqsWithAssignableBarcodes$FileNumber.txt" or die$!;
		...
		open SORTED, ">TempFiles/SubTemp_$number/GoodSeqsWithBarcodesSorted.txt" or die$!;
		...
		open GOODSEQSFILE, "TempFiles/SubTemp_$number/GoodSeqsWithBarcodesSorted.txt" or die$!;
			
		my $CurrentGoodBarcode = "RandomStartingBarcodeSeq";
			
		open CURRENTWRITEFILE, ">TempFiles/SubTemp_$number/IndividualRandomStartingBarcodeSeq.txt" or die$!;
		my $aligncounter = 0;
		...
		while (<GOODSEQSFILE>)  {
			...
			if ($TempBarcode !~ /$CurrentGoodBarcode/)  {
				...
				open CURRENTWRITEFILE, ">>TempFiles/SubTemp_$number/Individual$NewIndividualName.txt" or die$!;
		...
		system "rm TempFiles/SubTemp_$number/IndividualRandomStartingBarcodeSeq.txt";
		close CURRENTWRITEFILE;	
		close REPORT; 
	...
	############################################################################################################################################################################
		...
		#open UNIQUESFORPLOT, ">TempFiles/SubTemp_$number/UniqueCountsForPlot.txt" or die$!;
		#close UNIQUESFORPLOT;
	
		print "\nIdentifying unique sequences for each individual.\n";
		
		open SAMPLENAMESFORPLOT, ">TempFiles/SubTemp_$number/SampleNamesForPlot.txt" or die$!;
			...
			foreach my $g (@CurrentFileSampleNames)  {
				...
				if (-e "TempFiles/SubTemp_$number/Individual$g.txt")  {			#Check to make sure the file exists (the individual had at least one unique read that passed all of the filtering).  Without this check, the program will crash if there is an individual with no reads (Individual$g file doesn't exist).
				
				
					open UNSORTED, "TempFiles/SubTemp_$number/Individual$g.txt" or die$!;
					...
					open SORTED, ">TempFiles/SubTemp_$number/SortedIndividual$g.txt" or die$!;
					...
					system "uniq -c TempFiles/SubTemp_$number/SortedIndividual$g.txt TempFiles/SubTemp_$number/UniqueWithCountsIndividual$g.txt";
					...
					system "wc -l TempFiles/SubTemp_$number/UniqueWithCountsIndividual$g.txt | tee -a Output/RunInfo/Report_$SubFileNamesWExtension[$FileNumber-1].txt"; #TempFiles/UniqueCountsForPlot.txt";
		...
		system "rm TempFiles/SubTemp_$number/AllGoodSeqsWithAssignableBarcodes$FileNumber.txt";
		system "rm TempFiles/SubTemp_$number/SortedIndividual*.txt";
		system "rm TempFiles/SubTemp_$number/Individual*";
	...
	foreach my $number (0..@DirectoriesNumbers-1)  {
		system "mv TempFiles/SubTemp_$number/GoodSeqsNoBarcodes.txt TempFiles/GoodSeqsNoBarcodes_$number.txt";
	}

	system "cat TempFiles/GoodSeqsNoBarcodes_* > TempFiles/GoodSeqsNoBarcodes.txt";
	...
	foreach my $number (0..@DirectoriesNumbers-1)  {
		...
		mkdir "TempFiles/SubTemp_$number/TempUniqueInds";

		system "mv TempFiles/SubTemp_$number/UniqueWith* TempFiles/SubTemp_$number/TempUniqueInds";
		
		opendir UNIQUES, "TempFiles/SubTemp_$number/TempUniqueInds";
		...
		foreach my $uni (@UniquesFileNames)  {
			...
			if (exists($ParallelUniquesHash{$uni}))  {
				system "cat TempFiles/SubTemp_$number/TempUniqueInds/$uni >> TempFiles/$uni";
				system "cat TempFiles/SubTemp_$number/TempUniqueInds/$uni >> Output/$uni";
				$ParallelUniquesHash{$uni}++;
			}
			
			else {
				$ParallelUniquesHash{$uni} = 1;
				system "cp TempFiles/SubTemp_$number/TempUniqueInds/$uni TempFiles/$uni";
				system "cp TempFiles/SubTemp_$number/TempUniqueInds/$uni Output/$uni";
			}	
	...
	foreach my $key (keys(%ParallelUniquesHash)) {
		...
		if ($ParallelUniquesHash{$key} > 1) {
			...
			open UNIQUESDUPLICATED, "TempFiles/$key" or die$!;
			...
			system "rm TempFiles/$key";
			
			open UPDATEDFILE, ">TempFiles/$key" or die$!;
...
open GOODSEQS, "TempFiles/GoodSeqsNoBarcodes.txt" or die$!;
open OUT, ">TempFiles/AllUniqueSeqsToSort.txt" or die$!;
...
system "sort TempFiles/AllUniqueSeqsToSort.txt -o TempFiles/AllUniquesSorted.txt";
...
if ($MaxProcesses >1 )  {
	...
	foreach my $value (0..$MaxProcesses-2)  {
		...
		mkdir "TempFiles/Process$value";
		
		open PAIRSSUBSET, ">TempFiles/Process$value/TempFastaB_Subset.txt" or die$!;
	...
	mkdir "TempFiles/Process$MaxProcessesMinusOne";
	
	open PAIRSSUBSETFINAL, ">TempFiles/Process$MaxProcessesMinusOne/TempFastaB_Subset.txt" or die$!;
	...
	foreach my $folder (@FolderNamesForFork)  {
		...
		my @ForAlignmentSub = ();
		open FORARRAY, "TempFiles/$folder/TempFastaB_Subset.txt" or die$!;
		...
		open ACANAOUTFILEA, ">TempFiles/$folder/ACANAOutfileA.txt";
	
		#print "Created file\n\n";
		
		for (0..$ComparisonsSub-1)  {
		
			open SEQSTOALIGN, ">TempFiles/$folder/SeqsToAlign.txt" or die$!;
			...
			system "./ACANA  -I TempFiles/$folder/SeqsToAlign.txt -O TempFiles/$folder/ACANAOut.txt" ;
			
			open TEMPALIGNFILE, "TempFiles/$folder/ACANAOut.txt";
	...
	mkdir "TempFiles/SubProcessAlignments";
	
	foreach my $process (0..$MaxProcesses-1)  {
		system "mv TempFiles/Process$process/ACANAOutfileA.txt TempFiles/SubProcessAlignments/ACANAOutfileA_$process.txt";
	}

	system "cat TempFiles/SubProcessAlignments/* > TempFiles/ACANAOutfileA.txt";
 ...
 if ($MaxProcesses > 1)  {
	...
	if ($MaxProcesses > $NextCandidateLocusNumber) {
		...
		foreach my $number (0..$NextCandidateLocusNumber-1)  {
			mkdir "TempFiles/MafftSubProcess_$number";
			#mkdir "TempFiles/SubTemp_$number";
	...
	foreach my $locusfile (1..$NextCandidateLocusNumber)  {
		...
		system "cp -r CandidateLoci/CandidateLocus$locusfile.txt TempFiles/MafftSubProcess_$filecounter";
	...
	foreach my $number (@DirectoriesNumbers)  {	
		...
		opendir LOCUSNAMES, "TempFiles/MafftSubProcess_$number";
		...
		foreach my $locusname (@SubLocusNames)  {
			 ...
			 open CURRENTLOCUSFILE, "TempFiles/MafftSubProcess_$number/$locusname" or die$!;
			 open TEMPFILETOREALIGN, ">TempFiles/MafftSubProcess_$number/SeqsToReAlign.txt" or die$!;
			 ...
			 system "mafft --quiet TempFiles/MafftSubProcess_$number/SeqsToReAlign.txt > TempFiles/MafftSubProcess_$number/mafftReAlignOut.txt" ;
			 
			 open REALIGNEDTOPRINT, "TempFiles/MafftSubProcess_$number/mafftReAlignOut.txt" or die$!;
...
open PARALOGS, "TempFiles/ParalogousLoci.txt" or die$!;
