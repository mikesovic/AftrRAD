...
mkdir "Output/Genotypes" unless(-d "Output/Genotypes");
	...
	while (<RSCRIPT>)  {
		...
		if ($_ =~ /^MinNumberAllelesGenotypedToRetainLocus/)  {
			print RSCRIPTOUT "MinNumberAllelesGenotypedToRetainLocus<-NumRowsInMatrix*$MinProportion\n";
				...
				print RSCRIPTOUT "write.table(SNPMatrixWithLociWithMinGenotypes, file = \"Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.Temp1.txt\", sep = \"\\t\",row.names=FALSE, col.names=FALSE)";
		}
	...
	while (<RSCRIPT>)  {
		...
		if ($_ =~ /^MinNumberAllelesGenotypedToRetainLocus/)  {
			print RSCRIPTOUT "MinNumberAllelesGenotypedToRetainLocus<-NumRowsInMatrix*$MinProportion\n";
				...
				print RSCRIPTOUT "write.table(SNPMatrixWithLociWithMinGenotypes, file = \"Output/Genotypes/SNPMatrix_$MinPercent.All.Temp1.txt\", sep = \"\\t\",row.names=FALSE, col.names=FALSE)";
		}
...
if ($MaxLocation == 0)  {
	open SNPMATRIXTEMP, "Output/Genotypes/SNPMatrix_$MinPercent.All.Temp1.txt" or die$!;
	
	open SNPMATRIX, ">Output/Genotypes/SNPMatrix_$MinPercent.All.Temp.txt" or die$!;
...
close SNPMATRIXTEMP;
close SNPMATRIX;
system "rm Output/Genotypes/SNPMatrix_$MinPercent.All.Temp1.txt";
	...
	open SNPMATRIXTEMP, "Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.Temp1.txt" or die$!;
	
	open SNPMATRIX, ">Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.Temp.txt" or die$!;
...
close SNPMATRIXTEMP;
close SNPMATRIX;
system "rm Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.Temp1.txt";
...
if ($MaxLocation == 0)  {
	open NODUPSUPDATED, ">Output/Genotypes/Haplotypes_$MinPercent.All.txt" or die$!;
}

else {
	open NODUPSUPDATED, ">Output/Genotypes/Haplotypes_$MinPercent.$MaxLocation.txt" or die$!;
}	
...
if ($MaxLocation == 0)  {
	...
	open SNPMATRIX, "Output/Genotypes/SNPMatrix_$MinPercent.All.Temp.txt" or die$!;
	open OUT, ">Output/Genotypes/SNPMatrix_$MinPercent.All.txt" or die$!;
	...
	open SNPMATRIX, "Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.Temp.txt" or die$!;
	open OUT, ">Output/Genotypes/SNPMatrix_$MinPercent.$MaxLocation.txt" or die$!;
...
system "rm Output/Genotypes/SNPMatrix_$MinPercent*Tem*.txt";


#Output monomorphic loci scored in the threshold proportion of samples.

open MONOMORPHICSWITHCOUNTS, "TempFiles/MonomorphicLociWithCounts.txt" or die$!;
open MONOMORPHICOUT, ">Output/Genotypes/Monomorphics_$MinPercent.txt" or die$!;
...
open MONOMORPHICS, "Output/Genotypes/Monomorphics_$MinPercent.txt" or die$!;
...
system "cp TempFiles/MasterReport.txt Output/RunInfo/MasterReportTemp.txt";

open TEMP, "Output/RunInfo/MasterReportTemp.txt" or die$!;
open OUT, ">Output/RunInfo/MasterReport.txt" or die$!;
...
system "rm Output/RunInfo/MasterReportTemp.txt";


#system "R --vanilla --slave < RScripts/PlotReadCountsEdit.R";

print "Individual SNPs, haplotypes, and monomorphic loci have been printed to files located in Output/Genotypes.\n";
